  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Data Kasus TBC</h1>
          <p className="text-gray-600 mt-1">Total {filteredCases.length} kasus tercatat</p>
        </div>
      ...
      <div className="card">
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Cari nama, alamat, atau ID kasus..."
              value={searchTerm}
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-gray-900">Detail Kasus</h2>
                <button
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-semibold text-gray-700">ID Kasus</label>
                  <p className="text-gray-900 font-mono">{selectedCaseData.id}</p>
                </div>
                <div>
                  <label className="text-sm font-semibold text-gray-700">Nama Pasien</label>
                  <p className="text-gray-900">{selectedCaseData.patientName}</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold text-gray-700">Usia</label>
                    <p className="text-gray-900">{selectedCaseData.age} tahun</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-700">Jenis Kelamin</label>
                    <p className="text-gray-900">{selectedCaseData.gender === 'L' ? 'Laki-laki' : 'Perempuan'}</p>
                  </div>
                </div>
                <div>
                  <label className="text-sm font-semibold text-gray-700">Alamat</label>
                  <p className="text-gray-900">{selectedCaseData.address}</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold text-gray-700">RT/RW</label>
                    <p className="text-gray-900">{selectedCaseData.wilayah.rt}/{selectedCaseData.wilayah.rw}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-700">Kelurahan</label>
                    <p className="text-gray-900">{selectedCaseData.wilayah.kelurahan}</p>
                  </div>
                </div>
                <div>
                  <label className="text-sm font-semibold text-gray-700">Kecamatan</label>
                  <p className="text-gray-900">{selectedCaseData.wilayah.kecamatan}</p>
                </div>
                <div>
                ...
                <div>
                  <label className="text-sm font-semibold text-gray-700">Tipe Resistensi</label>
                  <p className="text-gray-900">{selectedCaseData.resistanceType === 'none' ? 'Tidak Ada' : selectedCaseData.resistanceType}</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold text-gray-700">Tanggal Diagnosis</label>
                    <p className="text-gray-900">{formatDate(selectedCaseData.diagnosisDate)}</p>
                  </div>
                  {selectedCaseData.treatmentStartDate && (
                    <div>
                      <label className="text-sm font-semibold text-gray-700">Mulai Pengobatan</label>
                      <p className="text-gray-900">{formatDate(selectedCaseData.treatmentStartDate)}</p>
                    </div>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold text-gray-700">Anggota Keluarga</label>
                    <p className="text-gray-900">{selectedCaseData.householdMembers} orang</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-700">Kontak Disaring</label>
                    <p className="text-gray-900">{selectedCaseData.contactsScreened}/{selectedCaseData.householdMembers}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold text-gray-700">Koordinat</label>
                    <p className="text-gray-900 font-mono text-xs">{selectedCaseData.latitude.toFixed(6)}, {selectedCaseData.longitude.toFixed(6)}</p>
                  </div>
                </div>
              </div>
